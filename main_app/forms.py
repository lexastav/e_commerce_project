...
class RegistrationForm(forms.ModelForm):
    ...
    def clean_email(self):
        ...
        if not re.search(r"[^@]+@[^@]+\.[^@]+", email):
            raise forms.ValidationError('Похоже вы пытаетесь ввести все что угодно, но не адрес электронной почты')
    ...
    def clean_username(self):
        ...
        if User.objects.filter(username=username).exists():
            raise forms.ValidationError(f'Пользователь {username} уже зарегистрирован в системе')
        return username
